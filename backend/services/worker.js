...
const processDueReminders = async () => {
  ...
  try {
    const { rows } = await pool.query(`
      SELECT
        r.*,
        cr.client_name, cr.montant_restant, cr.statut AS creance_statut,
      ...
      WHERE r.scheduled_at <= NOW()
        AND r.sent = FALSE
        AND cr.statut NOT IN ('payee','annulee')
        AND cr.montant_restant > 0
      LIMIT 50
    `);

    if (!rows.length) return;
    console.log(`⏰ Worker Fluxio → ${rows.length} reminders`);
    ...
    for (const reminder of rows) {
      try {
        ...
        const msgMarchand =
          `🔔 *Rappel automatique Fluxio*\n\n` +
          `Type : ${reminder.type?.toUpperCase() || 'AUTO'}\n` +
        ...
        await pool.query(
          `INSERT INTO notifications (merchant_id, creance_id, type, message, wa_link)
...
const startReminderWorker = () => {
  console.log('🔄 Worker Fluxio démarré (60s)');
  processDueReminders();
